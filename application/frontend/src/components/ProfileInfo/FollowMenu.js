...
function FollowMenu({ followingProfileOwnerFlag, profile, onFollowHandler }) {
  ...
  followingProfileOwnerFlag === true
    ? (followButtonStyle = styles["unfollow-button"])
    : (followButtonStyle = styles["follow-button"]);
  return (
    <div
    ...
    >
      <button
        className={`${styles["follow-unfollow-button"]} ${followButtonStyle}`}
        ...
        <ul className={`${styles["dropdown-items"]} ${styles[menuClassname]}`}>
          <li>
            <Link
              ...
              to={`/Followers/${profile.profile_id}`}
            >
              Followers
            </Link>
          </li>
